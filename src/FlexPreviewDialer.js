...
export default class FlexPreviewDialer extends FlexPlugin {
 ...
 async init(flex, manager) {
    ...
    flex.AgentDesktopView.Panel2.Content.replace(<AgentComponent key="agent-component" />, {sortOrder: -1
    });
      ...
      // } else {
        ...
        const response = flex.Actions.invokeAction("StartOutboundCall", {
            ...
            //  if(response==="completed"){
               resolve( flex.Actions.invokeAction("CompleteTask", { sid: payload.task.sid }));
            //  }
  ...
  flex.Actions.addListener("BeforeCompleteTask", (payload, abortFunction) => {
    ...
    console.log('task Channel name before acceptTask is :: '+task.taskChannelUniqueName);
  ...
  // })
    ...
    flex.Actions.addListener("afterRejectTask", (payload, abortFunction) => {
      ...
      async function deleteTask() {
        const responseDB = await fetch('https://14a0-42-105-134-5.ngrok-free.app/vdl/deleteTask', {
    ...
    manager.workerClient.on("reservationCreated", reservation => {
      ...
      else if(reservation.task.taskChannelUniqueName === 'voice'&&reservation.task.attributes.direction!=='outbound')
      {
        const task=reservation.task;
        console.log(`Accept Task Payload[ From Country ] =======> ${task.attributes.from_country}`);
        ...
        console.log(`task channel is not peview so going in else block =======> ${task.taskChannelUniqueName}`);

        flex.AgentDesktopView.Panel2.Content.replace(
        //  <CRMContainer key="listbasedCRMComponent" uri="https://www.bing.com/" />);
        <CRMContainer key="listbasedCRMComponent" uriCallback={callbackURIval} />);
      }
      ...
      reservation.on('completed', acceptedReservation => {
        ...
        console.log('Call direction is :: '+reservation.task.attributes.direction)
